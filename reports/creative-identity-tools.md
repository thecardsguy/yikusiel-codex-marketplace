...
|---|---|---|---|
| `img` | Image gen — OpenAI gpt-image-2 + Gemini | API keys | Win unverified |
...
| `comfy-workflow-mcp` | Manage/validate **ComfyUI** workflows (local image/video gen) | local ComfyUI | Windows OK |
| `roboflow` | Computer-vision pipelines (dataset/label/train/infer) | Roboflow key | Windows OK |
| `fiftyone-skills` | CV dataset curation/evaluation | — | Windows OK |
| `image-studio-mcp` | Image editing/studio | API | Windows OK |
| `codex-video-vision` | Video review / vision analysis | API | Windows OK |
...
| `remotion` | Programmatic video from React/Remotion | — | Windows OK |
...
3. **`comfy-workflow-mcp`** — drive a local ComfyUI graph with ControlNet/IP-Adapter/pose+reference for fine-grained face/pose/scene control (you supply the ComfyUI setup).
